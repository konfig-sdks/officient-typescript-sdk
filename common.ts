...
import { readableStreamToString, OfficientError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new OfficientError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new OfficientError(e, e.response?.data)
                }
                if (innerError instanceof OfficientError) {
                    // Got "OfficientError" from the above try block
                    throw innerError;
                }
